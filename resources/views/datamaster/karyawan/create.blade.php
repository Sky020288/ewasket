<form action="{{ route('karyawan.store') }}" id="formcreateKaryawan" method="POST" enctype="multipart/form-data">
    @csrf
    <x-input-with-icon-label icon="ti ti-barcode" label="NRP" name="nik" />
    <x-input-with-icon-label icon="ti ti-credit-card" label="No. KTP" name="no_ktp" />
    <x-input-with-icon-label icon="ti ti-user" label="Nama Personel" name="nama_karyawan" />
    <div class="row">
        <div class="col-6">
    ...
    <x-select-label label="Kantor" name="kode_cabang" :data="$cabang" key="kode_cabang" textShow="nama_cabang" />
    <x-select-label label="Satker" name="kode_dept" :data="$departemen" key="kode_dept" textShow="nama_dept" upperCase="true" />
    </div>
    ...
    <div class="form-group mb-3">
        <label for="exampleFormControlInput1" style="font-weight: 600" class="form-label">Status Personel</label>
        <select name="status_karyawan" id="pendidikan_terakhir" class="form-select">
            <option value="">Status Personel</option>
