<form action="{{ route('departemen.update', Crypt::encrypt($departemen->kode_dept)) }}" method="POST" id="formDepartemen">
   @csrf
   @method('PUT')
   <x-input-with-icon label="Kode Satker" name="kode_dept" icon="ti ti-barcode" value="{{ $departemen->kode_dept }}" />
   <x-input-with-icon label="Nama Satker" name="nama_dept" icon="ti ti-building" value="{{ $departemen->nama_dept }}" />
